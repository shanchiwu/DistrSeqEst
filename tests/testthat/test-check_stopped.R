test_that("check_stopped works for two.sided without d2", {
  ...
  theta <- c(0.1, 0.2)
  out <- check_stopped(
    model = "glm", k = 50, N0 = 10, interest_term = 1:2, d1 = 0.1, theta = theta,
...
test_that("check_stopped works for beta.protect without d2", {
  ...
  theta <- c(0.2, 0.3)
  out <- check_stopped(
    model = "glm", k = 80, N0 = 20, interest_term = 1:2, d1 = 0.05, theta = theta,
...
test_that("check_stopped handles auc_var and d2 properly", {
  ...
  theta <- c(0.1, 0.2)
  out <- check_stopped(
    model = "glm", k = 80, N0 = 10, interest_term = 1:2, d1 = 0.1, d2 = 0.05,
